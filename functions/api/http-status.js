...
import { checkHttpStatus } from "../../src/tools/http-status/index.js";
import {
  success,
  error,
  corsPreflight,
...
export async function onRequest(context) {
  ...
  try {
    ...
    const data = await checkHttpStatus(target);
    // Do not cache aggressively — status can change quickly
    ...
    return error(
      "INTERNAL_ERROR",
      "An unexpected error occurred while checking HTTP status.",
      500,
      { request, requestId }
    );
  }
