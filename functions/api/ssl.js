 ...
 * Observable HTTPS/HSTS configuration (not full TLS certificate audit).
 */

import { analyzeSsl } from "../../src/tools/ssl/index.js";
import {
  success,
  error,
  corsPreflight,
...
export async function onRequest(context) {
  ...
  try {
    ...
    const data = await analyzeSsl(target);
    ...
    return error(
      "INTERNAL_ERROR",
      "An unexpected error occurred while analyzing HTTPS configuration.",
      500,
      { request, requestId }
    );
  }
